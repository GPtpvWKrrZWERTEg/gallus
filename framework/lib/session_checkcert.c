...
static gallus_mutex_t s_lock;

static gallus_hashmap_t s_pat_tbl;
  ...
  if (IS_VALID_STRING(pat) == true &&
    ...
    *retpat = NULL;
    ...
    if (st == 0) {
      ...
      gallus_msg_error("regcomp(\"%s\") error: %s\n", pat, errbuf);
...
s_create_pair(conn_permission_t perm,
  ...
  if (perm == conn_allow || perm == conn_deny ||
    ...
    if (IS_VALID_STRING(issuer) == true) {
      if ((s = s_compile(issuer, &ri)) == GALLUS_RESULT_OK) {
    ...
    if (IS_VALID_STRING(subject) == true) {
      if ((s = s_compile(subject, &rs)) == GALLUS_RESULT_OK) {
    ...
    if ((ret = (pattern_pair_t *)malloc(sizeof(*ret))) != NULL) {
      ...
      gallus_perror(GALLUS_RESULT_NO_MEMORY);
    }
  } else {
    gallus_perror(GALLUS_RESULT_INVALID_ARGS);
  }
...
static inline void
s_destroy_tbl(void) {
  gallus_hashmap_destroy(&s_pat_tbl, true);
}


static inline gallus_result_t
s_clear_tbl(void) {
  return gallus_hashmap_clear(&s_pat_tbl, true);
}
...
s_add_pair(pattern_pair_t *p) {
  void *v = (void *)p;
  (void)gallus_hashmap_add(&s_pat_tbl, (void *)p, &v, true);
}


static bool
s_serilize_proc(void *key, void *val, gallus_hashentry_t he, void *arg) {
  bool ret = false;
...
s_serialize_pairs(size_t *szptr) {
  ...
  if ((n_pats = (size_t)gallus_hashmap_size(&s_pat_tbl)) > 0) {
...
s_checkcert_init(void) {
  if (s_create_tbl() != GALLUS_RESULT_OK) {
    gallus_exit_fatal("Can't initialize a hashmap for certificate check.\n");
  }
  if (gallus_mutex_create(&s_lock) != GALLUS_RESULT_OK) {
    gallus_exit_fatal("Can't initialize a mutex for certificate check.\n");
  }
}
...
s_checkcert_final(void) {
  ...
  s_destroy_tbl();
  if (s_lock != NULL) {
    (void)gallus_mutex_destroy(&s_lock);
  }
}
                ...
                if (strcasecmp(*tp, "self-signed") == 0 ||
                  ...
                  if (IS_VALID_STRING(issuer) == false) {
                    ...
                    if (IS_VALID_STRING(*tp) == true) {
                      if (gallus_str_unescape(*tp, "\"'", &issuer) > 0) {
                        do_create = true;
                      } else {
                  ...
                  if (IS_VALID_STRING(subject) == false) {
                    tp++;
                    if (IS_VALID_STRING(*tp) == true) {
                      if (gallus_str_unescape(*tp, "\"'", &subject) > 0) {
                        do_create = true;
                      } else {
                        gallus_msg_error("\"%s\": line " PFSZ(u) ": "
                                         "Invalid escape: '%s'.\n",
...
s_check_certificates_set_config_file(const char *filename) {
  (void)gallus_mutex_lock(&s_lock);
  snprintf(s_filename, sizeof(s_filename), "%s", filename);
  s_last_open = 0;
  (void)gallus_mutex_unlock(&s_lock);
}
  ...
  if (has_issuer == true || has_subject == true) {
    ...
    (void)gallus_mutex_lock(&s_lock);
    {
      if (IS_VALID_STRING(s_filename) == true) {
        if ((ret = s_load_config(s_filename)) != GALLUS_RESULT_OK) {
          gallus_perror(ret);
          gallus_msg_error("Can't load: '%s'.\n", s_filename);
          /*
           * for failsafe.
           */
      ...
      if (s_pats != NULL && s_n_pats > 0) {
        ...
        for (i = 0; i < s_n_pats; i++) {
          ...
          if (p != NULL) {
            ...
            if (perm != conn_unknown) {
              ret = (perm == conn_allow) ?
                    GALLUS_RESULT_OK : GALLUS_RESULT_NOT_ALLOWED;
              break;
            }
          }
        }
