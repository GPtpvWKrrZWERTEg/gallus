...
#define IS_GALLUS_RESULT_OK(s)	(((s) >= 0) ? true : false)

#define NSEC_TO_USEC(nsec) \
  (gallus_chrono_t)((((nsec) / 100LL) + 5LL) / 10LL)
...
#define NSEC_TO_TV(nsec, tv)                                      \
  do {                                                            \
    gallus_chrono_t __u_sec__ = NSEC_TO_USEC(nsec);              \
...
#define TS_TO_NSEC(ts) \
  ((gallus_chrono_t)((ts).tv_sec) * 1000LL * 1000LL * 1000LL + \
   (gallus_chrono_t)(ts).tv_nsec)

#define TV_TO_NSEC(tv) \
  (((gallus_chrono_t)((tv).tv_sec) * 1000LL * 1000LL + \
    (gallus_chrono_t)(tv).tv_usec) * 1000LL)
