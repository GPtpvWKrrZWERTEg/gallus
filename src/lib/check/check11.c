  ...
  if (likely(spec != NULL &&
    ...
    if (IS_VALID_STRING(cstr) == true) {
      ...
      if ((gallus_str_tokenize_with_limit(cstr, tokens, 6, 6, ":|")) == 6) {
        ...
        if ((ret = gallus_str_parse_uint64(tokens[0], &n_workers)) ==
            GALLUS_RESULT_OK &&
            (ret = gallus_str_parse_uint64(tokens[1], &n_qs)) ==
            GALLUS_RESULT_OK &&
            (ret = gallus_str_parse_uint64(tokens[2], &q_len)) ==
            GALLUS_RESULT_OK &&
            (ret = gallus_str_parse_uint64(tokens[3], &batch)) ==
            GALLUS_RESULT_OK) {
...
static gallus_chrono_t s_to = 100 * 1000;	/* 100 us. */
...
static gallus_statistic_t s_total_time = NULL;
static gallus_statistic_t s_total_clock = NULL;
static gallus_statistic_t s_tick = NULL;
static gallus_statistic_t s_throughput = NULL;
static gallus_statistic_t s_ev_time = NULL;
static gallus_statistic_t s_actual_meps = NULL;
static gallus_statistic_t *s_latencies = NULL;
...
s_init(void) {
  ...
  if (s_latencies != NULL) {
    ...
    (void)memset((void *)s_latencies, 0,
                 sizeof(gallus_statistic_t) * s_n_stages);
    for (i = 0; i < s_n_stages; i++) {
      snprintf(buf, 32, "latency[" PFSZ(u) "]", i);
      ret = gallus_statistic_create(&s_latencies[i], buf);
      if (ret != GALLUS_RESULT_OK) {
        goto done;
      }
    }
  ...
  if ((ret = gallus_statistic_create(&s_total_time, "total-time")) !=
      GALLUS_RESULT_OK) {
    goto done;
  }
  if ((ret = gallus_statistic_create(&s_total_clock, "total-clock")) !=
      GALLUS_RESULT_OK) {
    goto done;
  }
  if ((ret = gallus_statistic_create(&s_tick, "tick")) !=
      GALLUS_RESULT_OK) {
    goto done;
  }
  if ((ret = gallus_statistic_create(&s_throughput, "throughput")) !=
      GALLUS_RESULT_OK) {
    goto done;
  }

  if ((ret = gallus_statistic_create(&s_ev_time, "net load time")) !=
      GALLUS_RESULT_OK) {
    goto done;
  }
...
s_final(void) {
  ...
  if (s_latencies != NULL) {
    ...
    for (i = 0; i < s_n_stages; i++) {
      gallus_statistic_destroy(&s_latencies[i]);
    }

    free((void *)s_latencies);
  }

  gallus_statistic_destroy(&s_total_time);
  gallus_statistic_destroy(&s_total_clock);
  gallus_statistic_destroy(&s_tick);
  gallus_statistic_destroy(&s_throughput);
  gallus_statistic_destroy(&s_ev_time);
  gallus_statistic_destroy(&s_actual_meps);
}
    ...
    if (strcasecmp(*argv, "-ne") == 0) {
      ...
      if (IS_VALID_STRING(*argv) == true) {
        ...
        if ((ret = gallus_str_parse_uint64(*argv, &utmp)) ==
          ...
          gallus_msg_error("Invalid event size '%s'.\n", *argv);
          goto done;
        }
      } else {
        gallus_msg_error("An event size is not specified.\n");
        goto done;
      }
      ...
      if (IS_VALID_STRING(*argv) == true) {
        ...
        if ((ret = gallus_str_parse_uint64(*argv, &utmp)) ==
          ...
          gallus_msg_error("Invalid test run # '%s'.\n", *argv);
          goto done;
        }
      } else {
        gallus_msg_error("A test run # is not specified.\n");
        goto done;
      }
      ...
      if (IS_VALID_STRING(*argv) == true) {
        ...
        if ((ret = gallus_str_parse_uint64(*argv, &utmp)) ==
          ...
          if (utmp >= 2) {
            ...
            gallus_msg_error("The stage # must be >= 2.\n");
            ret = GALLUS_RESULT_TOO_SMALL;
            goto done;
          }
        } else {
          gallus_msg_error("Invalid stage # '%s'.\n", *argv);
          goto done;
        }
      } else {
        gallus_msg_error("A stage # is not specified.\n");
        goto done;
      }
      ...
      if (IS_VALID_STRING(*argv) == true) {
        ...
        if ((ret = gallus_str_parse_uint64(*argv, &utmp)) ==
          ...
          gallus_msg_error("Invalid weight # '%s'.\n", *argv);
          goto done;
        }
      } else {
        gallus_msg_error("A weight # is not specified.\n");
        goto done;
      }
      ...
      if (IS_VALID_STRING(*argv) == true) {
        ...
        if ((ret = gallus_str_parse_int64(*argv, &tmp)) ==
          ...
          gallus_msg_error("Invalid timeout '%s'.\n", *argv);
          goto done;
        }
      } else {
        gallus_msg_error("A timeout is not specified.\n");
        goto done;
      }
      ...
      if (IS_VALID_STRING(*argv) == true) {
        ...
        gallus_msg_error("Intermediate stages spec. in not specified.\n");
        goto done;
      }
      ...
      if (IS_VALID_STRING(*argv) == true) {
        ...
        gallus_msg_error("An egress stage spec. is not specified.\n");
        goto done;
      }
      ...
      if (IS_VALID_STRING(*argv) == true) {
        ...
        if ((ret = gallus_str_parse_uint64(*argv, &utmp)) ==
          ...
          gallus_msg_error("Invalid ingress worker # '%s'.\n", *argv);
          goto done;
        }
      } else {
        gallus_msg_error("An ingress worker # is not specified.\n");
        goto done;
      }
...
s_create(void) {
  ...
  if (s_stages == NULL) {
    ret = GALLUS_RESULT_NO_MEMORY;
    gallus_msg_error("Can't allocate an array of stage.\n");
    goto done;
  }
...
s_record(void) {
  ...
  if (ret == GALLUS_RESULT_OK) {
    ...
    gallus_chrono_t dummy1 = 0;
    gallus_chrono_t start_time = 0;
    gallus_chrono_t end_time = 0;
    gallus_chrono_t total_time = 0;
    ...
    (void)gallus_statistic_record(&s_total_time, (int64_t)total_time);
    (void)gallus_statistic_record(&s_total_clock, (int64_t)total_clock);
    (void)gallus_statistic_record(&s_tick, (int64_t)tick);
    (void)gallus_statistic_record(&s_throughput, (int64_t)meps);
    ...
    for (i = 0; i < s_n_stages - 1; i++) {
      ...
      (void)gallus_statistic_record(&s_latencies[i + 1], (int64_t)dummy0);
    }
  }
...
s_run(void) {
  ...
  if (s_stages != NULL) {
    size_t i;
    gallus_pipeline_stage_t *s;

    for (i = 0; i < s_n_stages; i++) {
      s = (gallus_pipeline_stage_t *)(&s_stages[i]);
      ret = gallus_pipeline_stage_setup(s);
      if (ret != GALLUS_RESULT_OK) {
        break;
      }
    }
    ...
    if (ret == GALLUS_RESULT_OK) {
      ...
      for (i = 0; i < s_n_stages; i++) {
        s = (gallus_pipeline_stage_t *)(&s_stages[i]);
        ret = gallus_pipeline_stage_start(s);
        if (ret != GALLUS_RESULT_OK) {
          break;
        }
      }
    ...
    if (ret == GALLUS_RESULT_OK) {
      ...
      if (ret == GALLUS_RESULT_OK) {
        ...
        if (ret == GALLUS_RESULT_OK) {
          ...
          for (i = 0; i < s_n_stages; i++) {
            s = (gallus_pipeline_stage_t *)(&s_stages[i]);
            ret = gallus_pipeline_stage_shutdown(s, SHUTDOWN_GRACEFULLY);
            if (ret != GALLUS_RESULT_OK) {
              break;
            }
          }
          if (ret == GALLUS_RESULT_OK) {
            ...
            if (ret == GALLUS_RESULT_OK) {
              ...
              for (i = 0; i < s_n_stages; i++) {
                s = (gallus_pipeline_stage_t *)(&s_stages[i]);
                ret = gallus_pipeline_stage_wait(s, -1LL);
                if (ret != GALLUS_RESULT_OK) {
                  break;
                }
              }
...
s_ref(size_t n_try) {
  ...
  if (dst != NULL && src != NULL) {
    ...
    size_t j;
    size_t k;
    gallus_chrono_t start = 0;
    gallus_chrono_t ev_start = 0;
    gallus_chrono_t end = 0;
    ...
    for (i = 0; i < n_try; i++) {
      ...
      if (sum == s_check_sum) {
        gallus_chrono_t total_time = end - start;
        gallus_chrono_t ev_time = end - ev_start;
        double meps = (double)s_n_events / (double)total_time * 1000000.0;
        double actual_meps = (double)s_n_events / (double)ev_time * 1000000.0;

        (void)gallus_statistic_record(&s_total_time, (int64_t)total_time);
        (void)gallus_statistic_record(&s_ev_time, (int64_t)ev_time);
        (void)gallus_statistic_record(&s_throughput, (int64_t)meps);
        (void)gallus_statistic_record(&s_actual_meps, (int64_t)actual_meps);
...
static inline void
s_print_statistic(FILE *fd, gallus_statistic_t *s,
  ...
  (void)gallus_statistic_min(s, &min);
  (void)gallus_statistic_max(s, &max);
  (void)gallus_statistic_average(s, &avg);
  (void)gallus_statistic_sd(s, &sd, false);
...
s_result(void) {
  ...
  if (s_do_ref == false) {
    ...
    (void)gallus_statistic_average(&s_tick, &tick);
    tick /= 1000000.0;
