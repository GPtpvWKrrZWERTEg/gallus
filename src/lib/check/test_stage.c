...
static inline void
s_test_stage_freeup(const gallus_pipeline_stage_t *sptr) {
  if (likely(sptr != NULL && *sptr != NULL)) {
    test_stage_t ts = (test_stage_t)(*sptr);

    if (ts->m_lock != NULL) {
      gallus_mutex_destroy(&(ts->m_lock));
    }
    if (ts->m_cond != NULL) {
      gallus_cond_destroy(&(ts->m_cond));
    }
...
s_test_stage_create(test_stage_t *tsptr,
  ...
  if (likely(tsptr != NULL &&
    ...
    if (likely((ret = gallus_mutex_create(&lock)) == GALLUS_RESULT_OK &&
...
s_test_stage_wait(const test_stage_t ts) {
  ...
  if (likely(ts->m_lock != NULL && ts->m_cond != NULL)) {
    ...
    (void)gallus_mutex_unlock(&(ts->m_lock));
...
s_test_stage_wakeup(const test_stage_t ts) {
  ...
  if (likely(ts->m_lock != NULL && ts->m_cond != NULL)) {
    ...
    {
      ts->m_do_exit = true;
      (void)gallus_cond_notify(&(ts->m_cond), true);
    }
    (void)gallus_mutex_unlock(&(ts->m_lock));
