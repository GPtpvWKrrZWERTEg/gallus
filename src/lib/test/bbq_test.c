...
typedef GALLUS_BOUND_BLOCK_Q_DECL(uint16_bbq, uint16_t) uint16_bbq;
typedef GALLUS_BOUND_BLOCK_Q_DECL(uint32_bbq, uint32_t) uint32_bbq;
typedef GALLUS_BOUND_BLOCK_Q_DECL(uint64_bbq, uint64_t) uint64_bbq;
typedef GALLUS_BOUND_BLOCK_Q_DECL(longdouble_bbq, long double) longdouble_bbq;
typedef GALLUS_BOUND_BLOCK_Q_DECL(char_bbq, char) char_bbq;
...
s_put(bbq *qptr, entry *put) {
  entry *pptr = put;
  return gallus_bbq_put(qptr, &pptr, entry *, TIMED_WAIT);
}

static gallus_result_t
s_get(bbq *qptr, entry **get) {
  return gallus_bbq_get(qptr, get, entry *, TIMED_WAIT);
}

static gallus_result_t
s_peek(bbq *qptr, entry **peek) {
  return gallus_bbq_peek(qptr, peek, entry *, TIMED_WAIT);
}

void
setUp(void) {
  gallus_result_t ret;
  ret = gallus_bbq_create(&bbQ, entry *, N_ENTRY, s_freeup);
...
test_bbq_creation(void) {
  ...
  ret = gallus_bbq_create(&bbqueue, entry *, N_ENTRY, NULL);
  if (ret != GALLUS_RESULT_OK) {
  ...
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "create bbq");

shutdown:
  gallus_bbq_shutdown(&bbqueue, false);
  gallus_bbq_destroy(&bbqueue, false);
}
...
test_bbq_creation_invalid_argument(void) {
  gallus_result_t ret;
  bbq bbqueue;
  ret = gallus_bbq_create(NULL, entry *, N_ENTRY, NULL);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret, "NULL bbq");
  ret = gallus_bbq_create(&bbqueue, entry *, -1, NULL);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret, "-1 entry");
  ret = gallus_bbq_create(&bbqueue, entry *, 0, NULL);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret, "0 entry");
}
...
test_bbq_put_get_invalid_argument(void) {
  ...
  ret = gallus_bbq_put(NULL, &(null_ptr), entry *, TIMED_WAIT);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret, "put - NULL bbq");
  ret = gallus_bbq_put(&bbQ, NULL, entry *, TIMED_WAIT);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret,
                            "put - NULL val_ptr");

  ret = gallus_bbq_get(NULL, &get, entry *, TIMED_WAIT);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret, "get - NULL bbq");
  ret = gallus_bbq_get(&bbQ, NULL, entry *, TIMED_WAIT);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret,
                            "get - NULL val_ptr");

  ret = gallus_bbq_peek(NULL, &get, entry *, TIMED_WAIT);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret, "peek - NULL bbq");
  ret = gallus_bbq_peek(&bbQ, NULL, entry *, TIMED_WAIT);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret,
                            "peek - NULL val_ptr");
}
...
test_bbq_put_get_ptr(void) {
  ...
  for (j = 0; j < N_LOOP; j++) {
    for (i = 0; i < N_ENTRY; i++) {
      ...
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "put entry");
    }
    for (i = 0; i < N_ENTRY; i++) {
      ret = s_peek(&bbQ, &get);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "peek entry");
...
test_bbq_put_get_null(void) {
  ...
  for (j = 0; j < N_LOOP; j++) {
    for (i = 0; i < N_ENTRY; i++) {
      ret = s_put(&bbQ, NULL);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "put NULL");
    }
    for (i = 0; i < N_ENTRY; i++) {
      ret = s_peek(&bbQ, &get);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "peek NULL");
      TEST_ASSERT_EQUAL_MESSAGE(NULL, get, "peek NULL - check value");
      ret = s_get(&bbQ, &get);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "get NULL");
...
test_bbq_put_get_8bit(void) {
  ...
  ret = gallus_bbq_create(&cbbq, char, N_ENTRY, NULL);
  if (ret != GALLUS_RESULT_OK) {
    gallus_perror(ret);
    goto shutdown;
  }
  ...
  for (j = 0; j < N_LOOP; j++) {
    for (i = 0; i < N_ENTRY; i++) {
      char c = (char)('a' + i);
      ret = gallus_bbq_put((&cbbq), &c, char, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "put entry");
    }
    for (i = 0; i < N_ENTRY; i++) {
      char c = (char)('a' + i);
      ret = gallus_bbq_peek((&cbbq), &get, char, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "peek entry");
      TEST_ASSERT_EQUAL_MESSAGE(c, get, "peek entry - check value");
      ret = gallus_bbq_get((&cbbq), &get, char, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "get entry");
...
shutdown:
  gallus_bbq_shutdown(&cbbq, false);
  gallus_bbq_destroy(&cbbq, false);
}
...
test_bbq_put_get_16bit(void) {
  ...
  ret = gallus_bbq_create(&ubbq, uint16_t, N_ENTRY, NULL);
  if (ret != GALLUS_RESULT_OK) {
    gallus_perror(ret);
    goto shutdown;
  }
  ...
  for (j = 0; j < N_LOOP; j++) {
    for (i = 0; i < N_ENTRY; i++) {
      ret = gallus_bbq_put((&ubbq), (uint16_t *)&i, uint16_t, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "put entry");
    }
    for (i = 0; i < N_ENTRY; i++) {
      ret = gallus_bbq_peek((&ubbq), (&get), uint16_t, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "peek entry");
      TEST_ASSERT_EQUAL_UINT16_MESSAGE(i, get, "peek entry - check value");
      ret = gallus_bbq_get((&ubbq), (&get), uint16_t, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "get entry");
...
shutdown:
  gallus_bbq_shutdown(&ubbq, false);
  gallus_bbq_destroy(&ubbq, false);
}
...
test_bbq_put_get_32bit(void) {
  ...
  ret = gallus_bbq_create(&ubbq, uint32_t, N_ENTRY, NULL);
  if (ret != GALLUS_RESULT_OK) {
    gallus_perror(ret);
    goto shutdown;
  }
  ...
  for (j = 0; j < N_LOOP; j++) {
    for (i = 0; i < N_ENTRY; i++) {
      ret = gallus_bbq_put((&ubbq), (uint32_t *)&i, uint32_t, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "put entry");
    }
    for (i = 0; i < N_ENTRY; i++) {
      ret = gallus_bbq_peek((&ubbq), (&get), uint32_t, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "peek entry");
      TEST_ASSERT_EQUAL_UINT32_MESSAGE(i, get, "peek entry - check value");
      ret = gallus_bbq_get((&ubbq), (&get), uint32_t, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "get entry");
...
shutdown:
  gallus_bbq_shutdown(&ubbq, true);
  gallus_bbq_destroy(&ubbq, true);
}
...
test_bbq_put_get_64bit(void) {
  ...
  ret = gallus_bbq_create(&ubbq, uint64_t, N_ENTRY, NULL);
  if (ret != GALLUS_RESULT_OK) {
    gallus_perror(ret);
    goto shutdown;
  }
  ...
  for (j = 0; j < N_LOOP; j++) {
    for (i = 0; i < N_ENTRY; i++) {
      ret = gallus_bbq_put((&ubbq), (uint64_t *)&i, uint64_t, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "put entry");
    }
    for (i = 0; i < N_ENTRY; i++) {
      ret = gallus_bbq_peek((&ubbq), (&get), uint64_t, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "peek entry");
      TEST_ASSERT_EQUAL_UINT64_MESSAGE(i, get, "peek entry - check value");
      ret = gallus_bbq_get((&ubbq), (&get), uint64_t, TIMED_WAIT);
      TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "get entry");
...
test_bbq_put_get_timedout(void) {
  ...
  for (i = 0; i < N_ENTRY; i++) {
    ret = s_put(&bbQ, NULL);
    TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "put-OK");
  }

  /* put timedout */
  for (i = 0; i < N_ENTRY; i++) {
    ret = s_put(&bbQ, NULL);
    TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_TIMEDOUT, ret, "put-TIMEDOUT");
  }

  /* peek ok */
  for (i = 0; i < N_ENTRY; i++) {
    ret = s_peek(&bbQ, &get);
    TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "peek-OK");
  }

  /* get ok */
  for (i = 0; i < N_ENTRY; i++) {
    ret = s_get(&bbQ, &get);
    TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "get-OK");
  }

  /* get timedout */
  for (i = 0; i < N_ENTRY; i++) {
    ret = s_get(&bbQ, &get);
    TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_TIMEDOUT, ret, "get-TIMEDOUT");
  }

  /* peek timedout */
  for (i = 0; i < N_ENTRY; i++) {
    ret = s_peek(&bbQ, &get);
    TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_TIMEDOUT, ret, "peek-TIMEDOUT");
  }
}
...
test_bbq_length_invalid_argument(void) {
  ...
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS,
                            gallus_bbq_is_empty(NULL, &result),
                            "is_empty(NULL)");
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS,
                            gallus_bbq_is_full(NULL, &result),
                            "is_full(NULL)");
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS,
                            gallus_bbq_size(NULL),
                            "length(NULL)");
}
...
test_bbq_length(void) {
  ...
  for (j = 0; j < N_LOOP; j++) {
    /* put ok */
    ret = gallus_bbq_is_empty(&bbQ, &result);
    TEST_ASSERT_EQUAL_MESSAGE(ret, GALLUS_RESULT_OK, "check is_empty");
    TEST_ASSERT_EQUAL_MESSAGE(true, result, "check is_empty");
    for (i = 0; i < N_ENTRY; i++) {
      TEST_ASSERT_EQUAL_MESSAGE(i, gallus_bbq_size(&bbQ), "check length");
      ret = gallus_bbq_is_full(&bbQ, &result);
      TEST_ASSERT_EQUAL_MESSAGE(ret, GALLUS_RESULT_OK, "check is_full");
    ...
    TEST_ASSERT_EQUAL_MESSAGE(N_ENTRY, gallus_bbq_size(&bbQ),
                              "check length");
    ret = gallus_bbq_is_full(&bbQ, &result);
    TEST_ASSERT_EQUAL_MESSAGE(ret, GALLUS_RESULT_OK, "check is_full");
    ...
    TEST_ASSERT_EQUAL_MESSAGE(0, gallus_bbq_size(&bbQ), "check length");
    ret = gallus_bbq_is_empty(&bbQ, &result);
    TEST_ASSERT_EQUAL_MESSAGE(ret, GALLUS_RESULT_OK, "check is_empty");
    TEST_ASSERT_EQUAL_MESSAGE(true, result, "check is_empty");
...
test_bbq_clear(void) {
  ...
  ret = gallus_bbq_clear(&bbQ, false);
  if (ret != GALLUS_RESULT_OK) {
    gallus_perror(ret);
  }
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "clear");
  TEST_ASSERT_EQUAL_MESSAGE(0, gallus_bbq_size(&bbQ), "check length");
  ret = gallus_bbq_is_empty(&bbQ, &result);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "check is_empty");
  TEST_ASSERT_EQUAL_MESSAGE(true, result, "check is_empty");
  ret = gallus_bbq_is_full(&bbQ, &result);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "check is_full");
  TEST_ASSERT_EQUAL_MESSAGE(false, result, "check is_full");
  ...
  for (j = 0; j < N_LOOP; j++) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(N_ENTRY, gallus_bbq_size(&bbQ), "check length");
    ret = gallus_bbq_is_empty(&bbQ, &result);
    TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "check is_empty");
    TEST_ASSERT_EQUAL_MESSAGE(false, result, "check is_empty");
    ret = gallus_bbq_is_full(&bbQ, &result);
...
test_bbq_clear_invalid_args(void) {
  gallus_result_t ret;
  ret = gallus_bbq_clear(NULL, false);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_INVALID_ARGS, ret, "clear NULL");
}
...
test_bbq_clear_freeup(void) {
  ...
  for (i = 0; i < N_ENTRY; i++) {
    ret = s_put(&bbQ, &put);
    TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "put");
  }
  ret = gallus_bbq_clear(&bbQ, false);
  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "clear");
...
s_test_bbq_shutdown_freeup_sub(bool create_with_freeup,
  ...
  if (create_with_freeup == true) {
    ret = gallus_bbq_create(&bbqueue, entry *, N_ENTRY, &s_freeup);
  } else {
    ret = gallus_bbq_create(&bbqueue, entry *, N_ENTRY, NULL);
  }

  TEST_ASSERT_EQUAL_MESSAGE(GALLUS_RESULT_OK, ret, "create bbqueue");
  for (i = 0; i < N_ENTRY; i++) {
    if (put_entry == true) {
      putptr = malloc(sizeof(entry));
      TEST_ASSERT_NOT_NULL_MESSAGE(putptr, " allocate putptr");
  ...
  if (shutdown_with_freeup == true) {
    gallus_bbq_shutdown(&bbqueue, true);
  } else {
    gallus_bbq_shutdown(&bbqueue, false);
  }
...
s_test_bbq_destroy_freeup_sub(bool create_with_freeup,
  ...
  if (destroy_with_freeup == true) {
    gallus_bbq_destroy(&bbqueue, true);
  } else {
    gallus_bbq_destroy(&bbqueue, false);
  }
...
s_run_put(void *arg) {
  ...
  for (i = 0; i < tv->putget_loop; i++) {
    ...
    if (shutdowned) {
      TEST_ASSERT_EQUAL_MESSAGE(ret, GALLUS_RESULT_NOT_OPERATIONAL,
                                "put: shutdowned");
    } else {
      switch (ret) {
...
test_bbq_safe_shutdown(void) {
  ...
  ret = gallus_bbq_is_operational(&bbQ, &result);
  TEST_ASSERT_EQUAL_MESSAGE(ret, GALLUS_RESULT_OK, "check is_operational");
