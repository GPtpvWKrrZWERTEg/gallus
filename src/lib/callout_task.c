...
static inline void
s_lock_task(gallus_callout_task_t t) {
  if (likely(t != NULL)) {
...
static inline void
s_unlock_task(gallus_callout_task_t t) {
  if (likely(t != NULL)) {
...
static inline void
s_wait_task(gallus_callout_task_t t) {
  if (likely(t != NULL)) {
    (void)gallus_cond_wait(&(t->m_cond), &(t->m_lock), -1LL);
  }
}


static inline void
s_wakeup_task(gallus_callout_task_t t) {
  if (likely(t != NULL)) {
    (void)gallus_cond_notify(&(t->m_cond), true);
  }
}
...
s_get_task_state_in_table(const gallus_callout_task_t t) {
  ...
  if (likely(t != NULL)) {
    ...
    r = gallus_hashmap_find(&s_tsk_tbl, (void *)t, &val);
...
static inline callout_task_state_t
s_set_task_state_in_table(const gallus_callout_task_t t,
  ...
  if (likely(t != NULL)) {
    ...
    if (likely((r = gallus_hashmap_find(&s_tsk_tbl, (void *)t, &val)) ==
               GALLUS_RESULT_OK)) {
      ret = (callout_task_state_t)val;
      val = (void *)s;
...
static inline void
s_delete_task_in_table(const gallus_callout_task_t t) {
  if (likely(t != NULL)) {
    (void)gallus_hashmap_delete(&s_tsk_tbl, (void *)t, NULL, true);
  }
}
...
s_runnable_run(const gallus_runnable_t *rptr, void *arg) {
  gallus_result_t ret = GALLUS_RESULT_ANY_FAILURES;

  (void)arg;

  if (likely(rptr != NULL && *rptr != NULL)) {
    gallus_callout_task_t t = (gallus_callout_task_t)*rptr;
...
s_create_task(gallus_callout_task_t *tptr,
  ...
  if (likely((ret = gallus_mutex_create_recursive(&rcrmtx)) ==
             GALLUS_RESULT_OK)) {
    if (unlikely((ret = gallus_cond_create(&cnd)) != GALLUS_RESULT_OK)) {
      gallus_mutex_destroy(&rcrmtx);
      goto done;
    }
  } else {
    goto done;
  ...
  if (ret != GALLUS_RESULT_OK && nm != NULL) {
    free((void *)nm);
  }
...
static inline void
s_delete(gallus_callout_task_t t) {
  gallus_runnable_t r = (gallus_runnable_t)t;
  ...
  if (likely(st == TASK_STATE_CANCELLED)) {
    ...
    if (likely(did_delete == true)) {
      gallus_mutex_destroy(&(t->m_lock));
      gallus_cond_destroy(&(t->m_cond));

      gallus_runnable_destroy(&r);
    }
  }
}
...
static inline void
s_destroy_task_no_lock(gallus_callout_task_t t) {
  if (likely(t != NULL)) {
    s_delete(t);
  }
}
...
static inline void
s_destroy_task(gallus_callout_task_t t) {
  if (likely(t != NULL)) {
...
static inline void
s_set_cancel_and_destroy_task_no_lock(gallus_callout_task_t t) {
  if (likely(t != NULL)) {
...
static inline void
s_set_cancel_and_destroy_task(gallus_callout_task_t t) {
  if (likely(t != NULL)) {
...
static inline void
s_cancel(const gallus_callout_task_t t) {
  if (likely(t != NULL)) {
...
static inline void
s_cancel_task_no_lock(const gallus_callout_task_t t) {
  if (likely(t != NULL)) {
    s_cancel(t);
  }
}
...
static inline void
s_cancel_task(gallus_callout_task_t t) {
  if (likely(t != NULL)) {
...
static inline gallus_result_t
s_exec_task(gallus_callout_task_t t) {
  gallus_result_t ret = GALLUS_RESULT_ANY_FAILURES;

  gallus_msg_debug(4, "exec enter.\n");
...
static inline gallus_result_t
s_submit_task(gallus_callout_task_t t,
              gallus_chrono_t initial_delay,
              gallus_chrono_t interval) {
  gallus_result_t ret = GALLUS_RESULT_ANY_FAILURES;

  if (likely(t != NULL)) {

    gallus_callout_task_t tt = t;
...
static inline gallus_result_t
s_force_schedule_task(gallus_callout_task_t t) {
  gallus_result_t ret = GALLUS_RESULT_ANY_FAILURES;
  callout_task_state_t st = TASK_STATE_UNKNOWN;
  gallus_callout_task_t ta[1] = { t };
  gallus_chrono_t now;
  bool do_run = false;
